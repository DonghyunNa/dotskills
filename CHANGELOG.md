...
- Extracted four sub-skills with independent reuse value: `dev-clarify-requirements`, `dev-diagnose-repo`, `dev-compare-options`, `dev-plan-rollback`. Each can stand alone (e.g., `dev-diagnose-repo` is useful for onboarding or audits independent of legacy planning).
- Renamed for a unified `dev-` domain prefix across the catalog (these skills are all consumed during development work, so the shared prefix doubles as a namespace): `task-breakdown` → `dev-breakdown-tasks`, `repo-diagnose` → `dev-diagnose-repo`, `decision-compare-options` → `dev-compare-options`, `change-plan-rollback` → `dev-plan-rollback`. All renamed names follow Pattern 4 (`<domain>-<verb>-<object>`) from the README naming section.
